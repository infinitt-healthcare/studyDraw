...
{
	pCmdUI->SetCheck(m_logPen.lopnWidth.x == 3);
}
...
void CstudyDrawDoc::OnUpdateIndicatorColor(CCmdUI* pCmdUI)
{
	...
	else if (m_nMouse_Mode == 1)
	{
		switch (m_logPen.lopnColor)
		{
		case RGB(255, 0, 0):
...
void CstudyDrawDoc::OnSelectBmpFlower()
{
	...
	m_lpcwstrBmpId = (LPCWSTR)IDB_FLOWER;
}


void CstudyDrawDoc::OnSelectBmpNightsky()
{
	m_lpcwstrBmpId = (LPCWSTR)IDB_NIGHTSKY;
}
...
{
	pCmdUI->SetCheck(m_lpcwstrBmpId == (LPCWSTR)IDB_FLOWER);
}


void CstudyDrawDoc::OnUpdateSelectBmpNightsky(CCmdUI* pCmdUI)
{
	pCmdUI->SetCheck(m_lpcwstrBmpId == (LPCWSTR)IDB_NIGHTSKY);
}
